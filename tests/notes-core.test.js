...
test("Notes window is wired into the shell, not a standalone page", () => {
  ...
  assert.ok(html.indexOf("notes-core.js") < html.indexOf("startpage.js"));
  ...
  for (const hook of [
    ...
    assert.ok(app.includes(hook), `startpage.js is missing ${hook}`);
  }
});

test("Habitica is gone from the app source", () => {
  for (const file of ["index.html", "startpage.js", "startpage.css"]) {
